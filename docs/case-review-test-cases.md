...
| TC-104 | P0 乐观锁冲突 | 同一案例开两个标签页 | Tab A 批准后，Tab B 用旧 expectedVersion 再提交 | 服务端 409 ``Resolution version conflict: expected N, found M.``；UI 横幅(ab6c702 起)**展示该 version conflict detail + Refresh case 按钮**——冲突可感知、给出可操作动作 ✓ |
