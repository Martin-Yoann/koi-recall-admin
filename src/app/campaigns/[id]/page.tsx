...
type CampaignTab = 'overview' | 'products' | 'remedies' | 'cases';

function isCampaignTab(value: string): value is CampaignTab {
  return value === 'overview' || value === 'products' || value === 'remedies' || value === 'cases';
}
...
export default function CampaignDetailPage() {
  ...
  const [refreshingTab, setRefreshingTab] = useState<CampaignTab | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [notFound, setNotFound] = useState(false);
  const [activeTab, setActiveTab] = useState<CampaignTab>('overview');
  const requestIdRef = useRef(0);

  const load = useCallback(async (tab: CampaignTab, showPageLoading = false) => {
    const requestId = ++requestIdRef.current;
    setLoading(showPageLoading);
    ...
    try {
      ...
      if (requestId === requestIdRef.current) {
        setError(tab === 'cases' ? 'Failed to load campaign cases.' : 'Failed to load campaign.');
      }
    } finally {
      if (requestId === requestIdRef.current) {
  ...
  useEffect(() => {
    ...
    // Invalidate an in-flight request when the route changes or the component unmounts.
    return () => {
  ...
  const handleTabChange = (value: string) => {
    if (!isCampaignTab(value) || value === activeTab) return;
    setActiveTab(value);
    void load(value);
  };

  const handleRetry = () => {
    void load(activeTab, campaign === null);
  };
  ...
  return (
    <div className="container-content py-8 space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange} className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          {refreshingTab && (
            <span className="inline-flex items-center gap-1.5 text-xs text-text-tertiary">
              <RefreshCw className="h-3.5 w-3.5 animate-spin" /> Refreshing {refreshingTab}…
            </span>
          )}
        </div>
        {error && (
          ...
          <Card>
            <CardContent className="pt-6">
              {campaign.products.length === 0 ? (
                <p className="text-sm text-text-secondary">No products are associated with this campaign.</p>
              ) : (
        ...
        {/* Remedies Tab */}
        <TabsContent value="remedies" keepMounted>
        ...
        {/* Cases Tab */}
        <TabsContent value="cases" keepMounted>
          <Card>
