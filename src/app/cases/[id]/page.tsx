...
const CONSUMER_NOTIFIED_TRANSITIONS = [
  "rejected",
  "duplicate",
  "withdrawn",
  "closed",
    ...
    <div className="flex items-center gap-3 rounded-lg border px-3 py-2 text-xs">
      ...
      <span
        className={cn(
          ...
          UPLOAD_STATUS_STYLES[doc.uploadStatus] ??
      ...
      <span
        className={cn(
          "text-[10px] font-bold uppercase px-1.5 py-0.5 rounded shrink-0",
          SCAN_STATUS_STYLES[doc.scanStatus] ?? "bg-slate-100 text-slate-600",
        )}
      >
  ...
  if (!caseRef) {
    return (
      <div className="p-12 text-center">
        ...
        <Link
          href="/cases"
          className="text-brand-emerald hover:underline text-sm"
        >
          Back to Cases
        </Link>
      </div>
...
function CaseDetailContent({
  ...
  const [authError, setAuthError] = useState<"signin" | "forbidden" | null>(
    null,
  );
  ...
  const [resolutionTypeOverride, setResolutionTypeOverride] = useState<
    "replacement" | "refund" | null
  >(null);
  ...
  const [repOutcome, setRepOutcome] = useState<
  ...
  useEffect(() => {
    ...
    const dialog = needInfoOpen
      ? needInfoDialogRef.current
      : lightboxDialogRef.current;
    const closeButton = needInfoOpen
      ? needInfoCloseRef.current
  ...
  const refresh = useCallback(async () => {
    ...
    if (result.ok) {
      ...
      setActionError(
        result.error?.detail || "You do not have permission to view this case.",
      );
  ...
  const loadInitialData = useCallback(async () => {
    ...
    try {
      ...
      if (mountedRef.current) {
        setActionError(
          err instanceof Error ? err.message : "Failed to load the case.",
        );
      }
    } finally {
      if (mountedRef.current) {
   ...
   */
  const isConflictResult = (result: {
    status: number;
    error?: { type?: string };
  }) =>
    result.status === 409 ||
    result.error?.type === "conflict" ||
    result.error?.type?.endsWith("conflict") === true;

  const applyActionError = (
    result: {
      ok: false;
      status: number;
  ...
  const handleTransition = async (
    ...
    const label = next
      ...
      .join(" ");
    const currentLabel = formatWorkflowLabel(
      record.workflow?.currentStage ?? record.status,
    );
    ...
    const reasonRequiredForTarget =
      ...
      (next === "closed" &&
        record.resolution?.status !== "externally_completed");
    // Mirror the colour of the button the operator pressed (brand/red/amber/...).
    const accent = triggerButton
      ? getComputedStyle(triggerButton).backgroundColor
      : undefined;
    const ownerName =
      staff.find((s) => s.id === record.assignedToStaffUserId)?.displayName ??
      "Unassigned";
    ...
    const details: Array<{ label: string; value: React.ReactNode }> = [
      ...
      {
        label: "Submitted",
        value: formatAdminDateTimeWithYear(record.submittedAt),
      },
    ...
    if (result.ok) {
      ...
      applyActionError(
        result,
        `Transition to ${next} failed (${result.status})`,
      );
    }
    setSubmitting(false);
  };
  ...
  const submitNeedInfo = async () => {
    ...
    if (note.length < 10) {
      setActionError(
        "Please describe what the consumer should provide (at least 10 characters).",
      );
    ...
    if (result.ok) {
      ...
      applyActionError(
        result,
        `Request for information failed (${result.status})`,
      );
    }
    setSubmitting(false);
  };
  ...
  const submitReportabilityClose = async () => {
    ...
    if (result.ok) {
      ...
      setRepError(
        result.error?.detail || "Failed to close the reportability review.",
      );
    }
  ...
  const handleResolutionAction = async (action: ResolutionAction) => {
    ...
    const refundDollars =
      resolutionType === "refund" && refundAmount.trim()
        ? Number.parseFloat(refundAmount.trim())
        : NaN;
    const refundAmountMinor = Number.isFinite(refundDollars)
    ...
    if (action === "resolution:approve" && resolutionType === "refund") {
      if (refundAmountMinor === undefined || refundAmountMinor <= 0) {
        setActionError(
          "Refund approvals require a positive refund amount in dollars.",
        );
        return;
      }
    ...
    if (result.ok) {
      ...
      applyActionError(
        result,
        `${formatWorkflowLabel(action)} failed (${result.status})`,
      );
    }
    setSubmitting(false);
  };
      ...
      <div className="p-12 text-center">
        ...
        <p className="text-sm text-text-tertiary mb-4">
          {visibleAuthError === "signin"
            ...
            : actionError ||
              "Your staff role does not have permission to view this case."}
        </p>
  ...
  if (!record) {
    return (
      <div className="p-12 text-center">
        ...
        <p className="text-sm text-text-tertiary mb-4">
          {actionError || "The case details are not available yet."}
        </p>
        <div className="flex items-center justify-center gap-3">
          <button
            onClick={retryLoad}
  ...
  const canCompleteResolution = resolutionActions.includes(
    "resolution:complete",
  );
  ...
  const selectedInfoOptions = INFO_REQUEST_OPTIONS.filter((option) =>
    needInfoNote.includes(option),
  ).length;
  ...
  return (
    <div className="space-y-5 max-w-screen-2xl mx-auto">
      ...
      <section
        ...
        className="relative overflow-hidden rounded-2xl border border-slate-200 bg-surface-elevated shadow-sm"
      >
        <div
          className="absolute inset-y-0 left-0 w-1 bg-brand-emerald"
          aria-hidden="true"
        />
        <div className="relative p-5 sm:p-6">
          <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-4 flex flex-wrap items-center gap-2.5">
                <h1
                  id="case-title"
                  translate="no"
                  className="break-all text-2xl font-bold tracking-tight text-text-primary font-mono sm:text-3xl"
                >
                  {cse.caseReference}
                </h1>
            ...
            <div className="flex shrink-0 items-center justify-between gap-3 xl:flex-col xl:items-end">
              <p className="hidden text-[10px] font-semibold uppercase tracking-[0.18em] text-text-tertiary xl:block">
                Live record
              </p>
              <button
                type="button"
          ...
          <div className="mt-6 grid gap-2 border-t border-slate-100 pt-4 sm:grid-cols-3">
            <div className="rounded-xl bg-surface-secondary/70 p-3">
              ...
              <p className="mt-1.5 capitalize text-sm font-semibold text-text-primary">
                {formatWorkflowLabel(cse.workflow?.currentStage ?? cse.status)}
              </p>
            </div>
            <div className="rounded-xl bg-surface-secondary/70 p-3">
              <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-text-tertiary">
              ...
              <p className="mt-1.5 line-clamp-2 text-sm font-semibold text-text-primary">
                {cse.workflow?.nextAction || "Review case details"}
              </p>
            </div>
            <div className="rounded-xl bg-surface-secondary/70 p-3">
              <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-text-tertiary">
              ...
              <p className="mt-1.5 truncate text-sm font-semibold text-text-primary">
                {assignedStaff?.displayName || "Unassigned"}
              </p>
            </div>
          </div>
        </div>
      ...
      >
        <h2 id="case-metrics-title" className="sr-only">
        ...
        <div className="rounded-xl border border-slate-200 bg-surface-elevated p-4 shadow-sm">
          ...
          <p className="mt-1 text-[11px] text-text-tertiary">
            Visible events in the timeline
          </p>
        </div>
        <div
          className={cn(
        ...
        >
          <div className="flex items-center justify-between gap-2">
            ...
            <AlertTriangle
              className={cn(
                "h-4 w-4",
                cse.incidentFlag ? "text-red-500" : "text-text-tertiary",
              )}
              aria-hidden="true"
            />
          </div>
          ...
          <p className="mt-1 text-[11px] text-text-tertiary">
            {cse.incidentFlag ? "Reportability status" : "No incident flag"}
          </p>
        </div>
      </section>
        ...
        <div className="rounded-xl border border-red-300 bg-red-50 p-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-red-700 mt-0.5">
              This case is flagged with a safety incident (subtype:{" "}
              {cse.subtype.replace(/_/g, " ")}). Review reportability
      ...
      {(cse.status === "need_info" || infoRequest) && (
        <div className="rounded-xl border border-amber-300 bg-amber-50 p-4">
          ...
          {infoRequest ? (
            <>
              <p className="text-xs text-amber-900 mt-1.5 whitespace-pre-wrap leading-relaxed">
                {infoRequest.note}
              </p>
              <p className="text-[10px] text-amber-700 mt-2">
                Requested {formatAdminDateTime(infoRequest.at)} · the consumer
                sees this case as “Action required” with this message.
              </p>
            </>
          ) : (
            <p className="text-xs text-amber-700 mt-1">
              This case is waiting for consumer information, but no request note
              was recorded (legacy transition). Move it back to under review
              once the consumer responds.
            </p>
          )}
        </div>
      )}
        ...
        <div className="rounded-xl border border-brand-emerald/40 bg-brand-emerald-light p-4 flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-bold text-emerald-800">
              Consumer Information Received
            </p>
            <p className="text-xs text-emerald-700 mt-0.5">
              The requested details were provided and this case moved back to{" "}
              <span className="font-semibold capitalize">
                {infoReturned.nextStatus.replace(/_/g, " ")}
              </span>{" "}
              on {formatAdminDateTime(infoReturned.at)} — review the new
              information and continue the decision.
            </p>
          </div>
        </div>
      )}
        ...
        >
          <AlertTriangle
            className={cn(
              ...
              actionConflict ? "text-red-600" : "text-amber-600",
            )}
            aria-hidden="true"
          />
          ...
          <CardHeader className="border-b border-slate-100 pb-3">
            <CardTitle className="flex items-center gap-1.5 text-sm">
              <Package
                ...
                aria-hidden="true"
              />
              Campaign &amp; Claimed Products
            </CardTitle>
            <p
              data-slot="card-description"
              className="text-xs text-text-tertiary"
            >
              Verify campaign context, product matching and purchase signals
          ...
          <CardContent className="space-y-3">
            {cse.campaign && (
              <div className="flex items-center gap-2 flex-wrap text-xs">
                <span className="font-semibold text-text-primary">
                  {cse.campaign.title || cse.campaign.slug}
                </span>
                <span className="font-mono text-text-tertiary px-1.5 py-0.5 rounded bg-slate-50 border border-slate-200">
                  {cse.campaign.code}
                </span>
                <Link
            ...
            {cse.products && cse.products.length > 0 ? (
                  ...
                  <tbody>
                    {cse.products.map((product) => (
                      <tr key={product.id} className="border-b last:border-0">
                        <td className="px-3 py-2">
                          <span
                            className={cn(
                              ...
                              product.checkResult === "potential_match" &&
                                "bg-emerald-50 text-emerald-700",
                              product.checkResult === "manual_review" &&
                                "bg-amber-50 text-amber-700",
                              product.checkResult === "not_matched" &&
                                "bg-red-50 text-red-700",
                            )}
                          >
                            {product.checkResult.replace(/_/g, " ")}
                          </span>
                          {product.identificationMode &&
                            product.identificationMode !==
                              "product_identifiers" && (
                              <span className="block text-[10px] text-text-tertiary mt-1">
                                via{" "}
                                {product.identificationMode.replace(/_/g, " ")}
                              </span>
                            )}
                        </td>
                        ...
                        <td className="px-3 py-2 text-text-secondary">
                          {product.purchaseChannel.replace(/_/g, " ")}
                        </td>
                        ...
                        <td className="px-3 py-2">
                          {product.purchaseCorroboration ? (
                            <span
                              className={cn(
                                "text-[10px] font-bold uppercase px-1.5 py-0.5 rounded",
                                product.purchaseCorroboration === "verified" &&
                                  "bg-emerald-50 text-emerald-700",
                                product.purchaseCorroboration === "partial" &&
                                  "bg-amber-50 text-amber-700",
                                product.purchaseCorroboration === "conflict" &&
                                  "bg-red-50 text-red-700",
                                !["verified", "partial", "conflict"].includes(
                                  product.purchaseCorroboration,
                        ...
                        <td className="px-3 py-2">
                          {product.reasonCodes &&
                          product.reasonCodes.length > 0 ? (
                            <div className="flex flex-wrap gap-1">
                              {product.reasonCodes.map((code) => (
                                <span
                                  key={code}
                                  className="text-[10px] font-bold uppercase px-1.5 py-0.5 rounded bg-slate-100 text-slate-700"
                                >
                                  {code.replace(/_/g, " ")}
                                </span>
                              ))}
                          ...
                          {product.riskFlags && product.riskFlags.length > 0 ? (
                            <div className="flex flex-wrap gap-1">
                              {product.riskFlags.map((flag) => (
                                <span
                                  key={flag}
                                  className="text-[10px] font-bold uppercase px-1.5 py-0.5 rounded bg-red-50 text-red-700"
                                >
                                  {flag.replace(/_/g, " ")}
                                </span>
                              ))}
              ...
              <p className="text-xs text-text-tertiary">
                No claimed products were recorded on this case.
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <CardHeader className="border-b border-slate-100 pb-3">
          <CardTitle className="flex items-center gap-1.5 text-sm">
            <FileText
              ...
              aria-hidden="true"
            />
            Evidence ({cse.documents?.length ?? 0})
          </CardTitle>
          <p
            data-slot="card-description"
            className="text-xs text-text-tertiary"
          >
            Files submitted with the case, grouped by review purpose.
          </p>
        </CardHeader>
        <CardContent className="space-y-4">
          {EVIDENCE_CATEGORIES.map((category) => {
            const docs =
              cse.documents?.filter((d) => d.category === category.id) ?? [];
          ...
          {cse.documents &&
            cse.documents.some(
              (d) => !EVIDENCE_CATEGORIES.some((c) => c.id === d.category),
            ) && (
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-wide text-text-tertiary mb-2">
                  Other
                ...
                <div className="space-y-2">
                  {cse.documents
                    .filter(
                      (d) =>
                        !EVIDENCE_CATEGORIES.some((c) => c.id === d.category),
                    )
                    .map((doc) => (
                      <DocumentRow
                        key={doc.id}
          ...
          {(!cse.documents || cse.documents.length === 0) && (
            <div className="rounded-xl border border-dashed border-slate-200 bg-surface-secondary/50 px-4 py-8 text-center">
              <FileText
                className="mx-auto h-7 w-7 text-text-tertiary"
                aria-hidden="true"
              />
              ...
              <p className="mt-1 text-xs text-text-tertiary">
                Uploaded proof and incident evidence will appear here.
              </p>
            </div>
          )}
        </CardContent>
        ...
        <Card className="border-red-200 shadow-sm">
          <CardHeader className="border-b border-red-100 pb-3">
            ...
            <p
              ...
              className="text-xs text-text-tertiary"
            >
              Safety context and reportability evidence for this case.
            </p>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
              ...
              <div className="rounded-lg bg-surface-secondary/60 p-3">
                ...
                <p className="mt-1 font-semibold text-text-primary capitalize">
                  {cse.incident.injurySeverity?.replace(/_/g, " ") ?? "—"}
                </p>
              </div>
              <div className="rounded-lg bg-surface-secondary/60 p-3">
                <p className="text-text-tertiary">Medical treatment</p>
                <p className="mt-1 font-semibold text-text-primary capitalize">
                  {cse.incident.medicalTreatment?.replace(/_/g, " ") ?? "—"}
                </p>
              </div>
              <div className="rounded-lg bg-surface-secondary/60 p-3">
                ...
                <p className="mt-1 font-semibold text-text-primary">
                  {cse.incident.occurredAt
                    ...
                    : cse.incident.occurredDateUnknown
                      ? "Date unknown"
                      : "—"}
                </p>
              </div>
              <div className="rounded-lg bg-surface-secondary/60 p-3">
                ...
                <p className="mt-1 font-semibold text-text-primary capitalize">
                  {cse.incident.usedAsIntended?.replace(/_/g, " ") ?? "—"}
                </p>
              </div>
              <div className="rounded-lg bg-surface-secondary/60 p-3">
                <p className="text-text-tertiary">Failure mode</p>
                <p className="mt-1 font-semibold text-text-primary capitalize">
                  {cse.incident.failureMode?.replace(/_/g, " ") ?? "—"}
                </p>
              </div>
              <div className="rounded-lg bg-surface-secondary/60 p-3">
                <p className="text-text-tertiary">Treatment received</p>
                <p className="mt-1 font-semibold text-text-primary capitalize">
                  {cse.incident.medicalTreatmentReceived?.replace(/_/g, " ") ??
                    "—"}
                </p>
              </div>
              <div className="rounded-lg bg-surface-secondary/60 p-3">
                ...
                <p className="mt-1 font-semibold text-text-primary capitalize">
                  {cse.incident.unitType?.replace(/_/g, " ") ?? "—"}
                </p>
              </div>
            </div>
            <div className="flex flex-wrap gap-1.5">
              {cse.incident.eventTypes.map((type) => (
                <span
                  key={type}
                  className="text-[10px] font-bold uppercase px-1.5 py-0.5 rounded bg-red-50 text-red-600"
                >
                  {type.replace(/_/g, " ")}
                </span>
              ))}
                ...
                {cse.incident.reportability ? (
                  <p className="mt-1 font-semibold text-text-primary">
                    {cse.incident.reportability.status === "filed" ? (
                      <>
                        Filed with CPSC
                        {cse.incident.reportability.cpscReference
                          ? ` · ${cse.incident.reportability.cpscReference}`
                          : ""}
                      </>
                    ) : (
            ...
            {cse.incident.narrative !== undefined ? (
              <div className="rounded-lg border border-amber-200 bg-amber-50 p-3">
                <p className="text-[11px] font-semibold uppercase tracking-wide text-amber-800 mb-1 flex items-center gap-1">
                  ...
                  Narrative · decrypted · audited
                </p>
                <p className="text-xs text-text-primary leading-relaxed whitespace-pre-wrap">
                  {cse.incident.narrative}
                </p>
              </div>
            ) : (
              <p className="text-xs text-text-tertiary rounded-lg bg-surface-secondary/60 p-3 flex items-center gap-1.5">
                <Lock className="h-3 w-3" />
                Narrative is encrypted. Use “View raw PII” (compliance role) to
                decrypt — the read is audited.
              </p>
            )}
            {cse.incident.injuryDescription !== undefined ? (
              <div className="rounded-lg border border-amber-200 bg-amber-50 p-3">
                <p className="text-[11px] font-semibold uppercase tracking-wide text-amber-800 mb-1 flex items-center gap-1">
                  <Eye className="h-3 w-3" />
                  Injury description · decrypted · audited
                </p>
                <p className="text-xs text-text-primary leading-relaxed whitespace-pre-wrap">
                  {cse.incident.injuryDescription}
                </p>
              </div>
            ) : null}
          </CardContent>
        ...
        <Card className="border-violet-200 shadow-sm">
          <CardHeader className="border-b border-violet-100 pb-3">
            ...
            <p
              ...
              className="text-xs text-text-tertiary"
            >
              Complete both gates before moving this case to a closed state.
            </p>
          </CardHeader>
          <CardContent className="space-y-3">
            ...
            <div className="flex items-start gap-3 rounded-lg border p-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-sm font-semibold text-text-primary">
                  Resolution externally completed
                </p>
            ...
            {cse.incidentFlag && (
              <div className="flex items-start gap-3 rounded-lg border p-3">
                ...
                {!cse.incident?.reportability ||
                cse.incident.reportability.status !== "pending" ? null : (
                  <Link
                    href="/incidents"
                    className="shrink-0 rounded-md border px-2 py-1 text-xs font-semibold text-text-secondary cursor-pointer hover:bg-surface-secondary"
                  >
      ...
      <div className="grid items-start gap-5 lg:grid-cols-3">
          ...
          <CardContent className="space-y-2.5">
            ...
            {can("case.detail.read_pii_raw") && (
              ...
              >
                ...
                {viewingRawPii
                  ? "Hide raw PII (back to masked)"
                  : "View raw PII (audited)"}
              </button>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="border-slate-200 shadow-sm">
          <CardHeader className="border-b border-slate-100 pb-3">
            ...
            <p
              ...
              className="text-xs text-text-tertiary"
            >
              Choose the staff owner responsible for the next review step.
            </p>
          </CardHeader>
          <CardContent className="space-y-3">
            {assignedStaff ? (
              <div className="rounded-lg bg-emerald-50/60 border border-emerald-200 p-3">
                ...
                <p className="text-xs text-text-tertiary">
                  {assignedStaff.email} · {assignedStaff.role}
                </p>
                {cse.assignedAt && (
            ...
            <div className="flex gap-2">
              <label htmlFor="case-assignee" className="sr-only">
              ...
              <Select
                ...
                title={
                  can("case.assign")
                    ? undefined
                    : "Requires the case.assign permission (reviewer+)"
                }
                className="flex-1"
        ...
        <Card
          id="resolution-card"
          className="border-slate-200 shadow-sm scroll-mt-5"
        >
          <CardHeader className="border-b border-slate-100 pb-3">
            <CardTitle className="flex items-center gap-1.5 text-sm">
              <CheckCircle2
            ...
            <p
              ...
              className="text-xs text-text-tertiary"
            >
              Track the requested remedy and complete the approved outcome.
            </p>
          </CardHeader>
          <CardContent className="space-y-3">
            {resolution ? (
              <>
                <div className="grid grid-cols-2 gap-3 text-xs">
                  <div className="rounded-lg bg-surface-secondary/60 p-3">
                    ...
                    <p className="mt-1 font-semibold text-text-primary">
                      {formatWorkflowLabel(resolution.status)}
                    </p>
                  </div>
                  <div className="rounded-lg bg-surface-secondary/60 p-3">
                    ...
                    <p className="mt-1 font-semibold text-text-primary">
                      {resolution.requestedType
                        ? formatWorkflowLabel(resolution.requestedType)
                        : "—"}
                    </p>
                  </div>
                  <div className="rounded-lg bg-surface-secondary/60 p-3">
                    ...
                    <p className="mt-1 font-semibold text-text-primary">
                      {resolution.approvedType
                        ? formatWorkflowLabel(resolution.approvedType)
                        : "—"}
                    </p>
                  </div>
                  <div className="rounded-lg bg-surface-secondary/60 p-3">
                ...
                {resolution.refundAmountMinor !== null && (
                  <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs text-text-secondary">
                    ...
                    <span className="font-bold text-text-primary">
                      ${(resolution.refundAmountMinor / 100).toFixed(2)}
                    </span>{" "}
                    {resolution.currency ?? ""}
                  </div>
                )}
                ...
                {resolution.trackingNumber && (
                  <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs text-text-secondary">
                    ...
                    {resolution.shippedAt
                      ? ` · shipped ${formatAdminDateTimeWithYear(resolution.shippedAt)}`
                      : ""}
                  </div>
                )}
                ...
                {resolution.completedAt && (
                  <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-xs text-emerald-800">
                    ...
                    {formatAdminDateTimeWithYear(resolution.completedAt)}
                  </div>
                )}

                {resolutionActions.length > 0 ? (
                  <>
                    <label htmlFor="resolution-note" className="sr-only">
                      Resolution note
                    </label>
                    <Input.TextArea
                      id="resolution-note"
                    ...
                    {canApproveResolution ? (
                      <div className="space-y-2 rounded-lg border border-slate-200 bg-slate-50 p-3">
                        <p className="text-[11px] font-semibold uppercase tracking-wide text-text-tertiary">
                          Approval details
                          {resolution?.requestedType
                            ? " (pre-filled from the consumer’s request)"
                            : ""}
                        </p>
                        <div className="flex gap-2">
                          <label htmlFor="resolution-type" className="sr-only">
                            Resolution type
                          </label>
                          <Select
                            id="resolution-type"
                            ...
                            onChange={(val) =>
                              setResolutionTypeOverride(
                                val as "replacement" | "refund",
                              )
                            }
                            className="flex-1"
                            options={[
                    ...
                    {canCompleteResolution ? (
                      <>
                        <label htmlFor="external-reference" className="sr-only">
                          External reference for completion
                        </label>
                        <Input
                    ...
                    <div className="flex flex-wrap gap-2">
                      {resolutionActions.map((action) => (
                        <button
                          ...
                          disabled={
                            submitting || !can("case.status.transition")
                          }
                          title={
                            can("case.status.transition")
                              ? undefined
                              : "Requires the case.status.transition permission (reviewer+)"
                          }
                          className={cn(
                            ...
                            RESOLUTION_ACTION_STYLES[action] ??
                              "bg-slate-600 hover:bg-slate-700 text-white",
                          )}
                        >
                          {formatWorkflowLabel(
                            action.replace("resolution:", ""),
                          )}
                        </button>
                      ))}
                    </div>
              ...
              <p className="text-sm text-text-tertiary">
                No resolution record is available for this case.
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="border-slate-200 shadow-sm">
          <CardHeader className="border-b border-slate-100 pb-3">
            ...
            <p
              ...
              className="text-xs text-text-tertiary"
            >
              Move the case forward only after the required review evidence is
              complete.
            </p>
          </CardHeader>
          <CardContent className="space-y-3">
            ...
            {isTerminal ? (
              <p className="text-xs text-text-tertiary rounded-lg bg-surface-secondary/60 p-3">
                ...
                {cse.status.replace(/_/g, " ")}). No further transitions are
                allowed.
              </p>
            ) : (
              <>
                ...
                <label htmlFor="transition-reason" className="sr-only">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {!can("case.status.transition") && (
                    <p className="text-xs text-text-tertiary w-full">
                      Your role ({role ?? "signed out"}) is read-only —
                      transitions require a reviewer, compliance, or
      ...
      <Card className="border-slate-200 shadow-sm">
        <CardHeader className="border-b border-slate-100 pb-3">
          ...
          <p
            ...
            className="text-xs text-text-tertiary"
          >
            A chronological record of case, document and compliance activity.
          </p>
        </CardHeader>
        <CardContent>
          <div className="relative max-h-[320px] space-y-2 overflow-y-auto pl-1">
            {audit.map((e) => (
              <div
                key={e.id}
                className="relative flex items-start gap-3 border-b py-2 pl-3 last:border-0 before:absolute before:bottom-0 before:left-[0.45rem] before:top-5 before:w-px before:bg-slate-200 last:before:hidden"
                style={{ borderColor: "rgba(0,53,39,0.06)" }}
              >
                <div
                  className={cn(
                    "relative z-10 mt-2 h-2 w-2 shrink-0 rounded-full ring-4 ring-card",
                    AUDIT_DOT[e.resourceType] ?? "bg-slate-400",
                  )}
                />
                <div className="min-w-0 flex-1">
          ...
          >
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex min-w-0 items-start gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p
                      id="need-info-description"
                      className="mt-1 text-xs leading-relaxed text-text-tertiary"
                    >
                      Case{" "}
                      <span className="font-mono font-semibold text-text-secondary">
                        {cse.caseReference}
                      </span>{" "}
                      · the consumer will see this message in their case
                      workspace.
                    </p>
                  </div>
                </div>
                <button
                  ref={needInfoCloseRef}
                  type="button"
                  onClick={() => setNeedInfoOpen(false)}
                  disabled={submitting}
                  className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-text-tertiary transition-colors hover:bg-surface-secondary hover:text-text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-emerald/40 disabled:opacity-50"
                  aria-label="Close request information drawer"
                >
            ...
            <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-5 py-5 sm:px-6">
                ...
                <div className="flex items-start gap-2.5">
                  ...
                  <div>
                    <p className="text-xs font-semibold text-orange-900">
                      Ask only for information needed to complete the review.
                    </p>
                    <p className="mt-1 text-[11px] leading-relaxed text-orange-800/80">
                      This request is recorded in the case timeline and changes
                      the consumer-facing status to Action required.
                    </p>
                  </div>
                </div>
              </div>
              <div className="space-y-3">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-0.5 text-[11px] text-text-tertiary">
                      Select prompts to start your message.
                    </p>
                  </div>
                  <button
                    type="button"
                    onClick={() =>
                      setNeedInfoNote((prev) =>
                        selectedInfoOptions === INFO_REQUEST_OPTIONS.length
                          ? prev
                              .split("\n")
                              .filter(
                                (line) =>
                                  !INFO_REQUEST_OPTIONS.some(
                                    (option) => line.trim() === `• ${option}`,
                                  ),
                              )
                              .join("\n")
                              .trim()
                    ...
                    className="shrink-0 rounded-md px-2 py-1 text-[11px] font-semibold text-brand-emerald transition-colors hover:bg-brand-emerald-light focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-emerald/40"
                  >
                    {selectedInfoOptions === INFO_REQUEST_OPTIONS.length
                      ? "Clear all"
                      : "Select all"}
                ...
                <div
                  ...
                  role="group"
                  aria-label="Suggested information requests"
                >
              ...
              <div className="mt-6 space-y-2">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span
                    className={cn(
                      ...
                      needInfoNoteLength === 0 || needInfoNoteValid
                        ? "text-text-tertiary"
                        : "text-orange-700",
                    )}
                  >
                ...
                />
                <div
                  id="need-info-note-help"
                  className="flex items-start justify-between gap-3 text-[10px] leading-relaxed text-text-tertiary"
                >
                  <span>
                    {needInfoNoteValid
                      ? "Ready to send. Free-form edits are welcome."
                      : "Add at least 10 characters before sending."}
                  </span>
            ...
            <div className="flex shrink-0 flex-col-reverse gap-2 border-t border-slate-100 px-5 py-4 pb-[calc(1rem+env(safe-area-inset-bottom))] sm:flex-row sm:items-center sm:justify-between sm:px-6">
              <p className="text-[10px] text-text-tertiary">
                {selectedInfoOptions} of {INFO_REQUEST_OPTIONS.length}{" "}
              ...
              <div className="flex gap-2 sm:justify-end">
                <button
                  ...
                  onClick={() => setNeedInfoOpen(false)}
                  disabled={submitting}
                  className="flex-1 rounded-lg border border-slate-200 px-3 py-2.5 text-xs font-semibold text-text-secondary transition-colors hover:bg-surface-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-emerald/40 disabled:opacity-50 sm:flex-none"
                >
                  Cancel
                </button>
                <button
                  ...
                  disabled={submitting || !needInfoNoteValid}
                  className="inline-flex flex-1 items-center justify-center gap-1.5 rounded-lg bg-orange-600 px-3 py-2.5 text-xs font-semibold text-white transition-colors hover:bg-orange-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-400/50 disabled:opacity-50 sm:flex-none"
                >
                  {submitting ? (
                    <RefreshCw
          ...
          >
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex min-w-0 items-start gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p
                      id="evidence-preview-description"
                      className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[10px] text-slate-300"
                    >
                      <span>{lightbox.declaredMimeType}</span>
                      <span aria-hidden="true">·</span>
                      <span>
                        {(lightbox.sizeBytes / (1024 * 1024)).toFixed(2)} MB
                      </span>
                      <span aria-hidden="true">·</span>
                ...
                <button
                  ...
                  className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-white/10 text-white transition-colors hover:bg-white/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/60"
                ...
                {docAccess[lightbox.id] ? (
                  ...
                  <div className="max-w-md rounded-xl border border-white/10 bg-white/5 p-8 text-center text-sm text-slate-300">
                    ...
                    <p className="mt-1.5 text-xs leading-relaxed text-slate-400">
                      The signed access link may have expired, or this file is
                      not an image. Close this preview and try opening the
                      original file from its evidence row.
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="flex shrink-0 flex-col gap-3 border-t border-white/10 px-4 py-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] sm:flex-row sm:items-center sm:justify-between sm:px-5">
              <p className="text-[10px] text-slate-400">
                ...
                <kbd className="rounded border border-white/20 bg-white/10 px-1.5 py-0.5 font-mono text-slate-300">
                  Esc
                </kbd>{" "}
                to close
              </p>
                ...
                {docAccess[lightbox.id] && (
                  <>
                    <a
                      ...
                      download
                      className="inline-flex flex-1 items-center justify-center gap-1.5 rounded-lg bg-white/10 px-3 py-2 text-xs font-semibold text-white transition-colors hover:bg-white/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/60 sm:flex-none"
                    >
                    ...
                    <a
                      ...
                      className="inline-flex flex-1 items-center justify-center gap-1.5 rounded-lg bg-brand-emerald px-3 py-2 text-xs font-semibold text-white transition-colors hover:bg-brand-emerald-dark focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-emerald/60 sm:flex-none"
                    >
                      <ExternalLink
