...
function reportabilityLabel(incident: IncidentSummary): string {
  return (incident.reportability?.status ?? "none")
    .replace(/_/g, " ")
    .toUpperCase();
}
...
export default function IncidentsPage() {
  ...
  const [outcome, setOutcome] = useState<"filed" | "documented_non_reportable">(
    "filed",
  );
  ...
  const openReview = (incident: IncidentSummary) => {
    ...
    setRationale("");
    setCpscReference("");
    // Opening this dialog is a user event, so reading the clock here is safe: it is
    // a default the operator can overwrite, not a value the server would have had
    // to invent.
  ...
  const submitReview = async () => {
    ...
    if (outcome === "filed" && !filedAt.trim()) {
      setDialogError(
        "The date the filing was made is required when the outcome is Filed.",
      );
      return;
    }
    if (outcome === "filed" && filingEvidence.trim().length < 10) {
      setDialogError(
        "Say what the filing rests on — the receipt or acknowledgement (minimum 10 characters).",
      );
      return;
    }
    setSubmitting(true);
    ...
    if (result.ok) {
      ...
      setDialogError(
        result.error?.detail || "Failed to close reportability review.",
      );
    }
    setSubmitting(false);
  };
  ...
  const pending = useMemo(
    () =>
      incidents.filter(
        (incident) => incident.reportability?.status === "pending",
      ),
    [incidents],
  );
  const filed = useMemo(
    () =>
      incidents.filter(
        (incident) => incident.reportability?.status === "filed",
      ),
    [incidents],
  );
  const nonReportable = useMemo(
    () =>
      incidents.filter(
        (incident) =>
          incident.reportability?.status === "documented_non_reportable",
      ),
    [incidents],
  );
    ...
    <div className="space-y-5 max-w-screen-2xl mx-auto">
      <div className="flex items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-text-secondary mt-0.5">
            {incidents.length} incidents · {pending.length} pending
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          { label: "Filed (CPSC)", value: filed.length },
          { label: "Non-Reportable", value: nonReportable.length },
          { label: "Total", value: incidents.length },
        ].map((stat) => (
          <div
            key={stat.label}
            className="rounded-xl border bg-surface-elevated p-4 card-lift cursor-default"
          >
      ...
      <div className="rounded-xl border bg-surface-elevated overflow-hidden">
        ...
        {!isAuthenticated && !authLoading ? (
          <div className="text-center py-14">
            <Shield
              className="h-8 w-8 mx-auto text-text-tertiary mb-3"
              aria-hidden="true"
            />
            ...
            <p className="text-xs text-text-tertiary mb-4">
              Sign in to review incident reportability.
            </p>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {incidents.map((incident) => (
                  <tr
                    key={incident.id}
                    className="border-b hover:bg-surface-secondary transition-colors"
                  >
                    <td
                    ...
                    <td className="px-4 py-3">
                      <Link
                        href={`/cases/${encodeURIComponent(incident.caseReference)}`}
                        className="text-sm font-medium text-text-primary hover:text-brand-emerald hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-emerald/30 rounded-sm"
                      >
                    ...
                    <td className="px-4 py-3 text-xs text-text-secondary">
                      ...
                      {incident.medicalTreatment ? (
                        <span className="block text-[10px] text-text-tertiary mt-0.5">
                          treatment: {incident.medicalTreatment}
                        </span>
                      ) : null}
                    </td>
                    {/* Structured capture is absent on incidents filed before it existed, so '—' is a
                  real state here, not a loading placeholder. */}
                    <td className="px-4 py-3 text-xs text-text-secondary">
                      <span className="capitalize">
                        {incident.failureMode?.replace(/_/g, " ") ?? "—"}
                      </span>
                      {incident.unitType ? (
                        <span className="block text-[10px] text-text-tertiary mt-0.5">
                          unit: {incident.unitType}
                        </span>
                      ) : null}
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-1">
                        {incident.eventTypes.map((type) => (
                          <span
                            key={type}
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={cn(
                          ...
                          REPORTABILITY_STYLES[
                            incident.reportability?.status ?? ""
                          ] ?? "bg-slate-50 text-slate-700",
                        )}
                      >
                        {reportabilityLabel(incident)}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      {incident.reportability?.status === "pending" &&
                      can("review.close") ? (
                        <Button
                          type="primary"
                          size="small"
          ...
          <div className="text-center py-14">
            ...
            <p className="text-xs text-text-tertiary">
              {loading
                ? "Loading incidents…"
                : "Incidents will appear here when linked to cases."}
            </p>
          </div>
        )}
      </div>
      ...
      {reviewing ? (
        ...
        >
          <div className="w-full max-w-lg rounded-2xl shadow-2xl max-h-[calc(100vh-2rem)] flex flex-col overflow-hidden bg-[var(--surface-elevated)] backdrop-blur-md">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-text-tertiary mt-0.5">
                    Incident {shortId(reviewing.id)}… · Case{" "}
                    {reviewing.caseReference}
                  </p>
                </div>
              </div>
              <button
                ...
                className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg hover:bg-surface-secondary cursor-pointer transition-colors"
            ...
            <div
              className="px-6 py-5 space-y-4 overflow-y-auto min-h-0 flex-1"
            ...
            >
                ...
                <>
                  <div className="space-y-1.5">
                    ...
                    <Input
                      id="rep-cpsc"
                      value={cpscReference}
                      onChange={(e) => setCpscReference(e.target.value)}
                      placeholder="CPSC-2026-001"
                      maxLength={200}
                    />
                  </div>
                  <div className="space-y-1.5">
                    <label
                      htmlFor="rep-filed-at"
                      className="text-[11px] font-semibold uppercase tracking-wider text-text-secondary"
                    >
                  ...
                  <div className="space-y-1.5">
                    <label
                      htmlFor="rep-filing-evidence"
                      className="text-[11px] font-semibold uppercase tracking-wider text-text-secondary"
                    >
                      What the filing rests on
                    </label>
                    <Input.TextArea
                      id="rep-filing-evidence"
                      value={filingEvidence}
                      onChange={(e) => setFilingEvidence(e.target.value)}
                      placeholder="The receipt, acknowledgement or submission confirmation."
                      maxLength={2000}
                      autoSize={{ minRows: 3, maxRows: 6 }}
                    />
                    <p className="text-[10px] text-text-tertiary">
                      Minimum 10 characters · encrypted at rest
                    </p>
                  </div>
                </>
              )}
              ...
              <div className="space-y-1.5">
                ...
                <Input.TextArea
                  ...
                  placeholder="Explain the decision (minimum 10 characters)."
                  maxLength={2000}
                  autoSize={{ minRows: 4, maxRows: 8 }}
                />
                <p className="text-[10px] text-text-tertiary">
                  Minimum 10 characters · recorded in the audit trail
                </p>
              </div>
            ...
            <div
              className="shrink-0 flex justify-end gap-2 px-6 py-4 border-t bg-[var(--surface-elevated)]"
              style={{ borderColor: "var(--border)" }}
            >
              <Button
                onClick={() => setReviewing(null)}
